...
import re, os, uuid, shutil, asyncio, tempfile
...
def process_media_upload(task_id: str, folder_path: str, class_text: str):
    ...
    try:
        ...
        media_files, txt_file = get_files_from_folder(folder_path=final_path)
        ...
        uploader.run_full_workflow(class_text=class_text, file1_path=media_files[0], file2_path=media_files[1],
                                   post_title=summary, title=summary, description=description)
...
async def upload_folder(request: UploadRequest):
    ...
    try:
        ...
        # Add to queue
        await task_queue.add_task(task_id, process_media_upload, task_id, request.folder_path, request.class_text)
...
@app.post('/upload-pdf')
async def upload_pdf(post_title: str = Form(...),
                     title: str = Form(...),
    ...
    try:
        ...
        # Add to queue
        await task_queue.add_task(task_id, process_pdf_upload, task_id, tmp_path, post_title, title, description,
                                  class_text)
